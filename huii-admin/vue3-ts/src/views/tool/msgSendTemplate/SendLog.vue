<template>
  ...
  <el-form :inline="true" :size="size" v-show="showSearch">
    ...
    <el-form-item label="发送时间" class="global-input-item">
      <el-date-picker
          type="datetimerange"
          v-model="time"
          range-separator="至"
  ...
  <el-form :inline="true" :size="size">
    ...
    <el-form-item class="global-form-item-margin" v-if="checkPermission('msg:send:log:delete')">
    ...
    <!--export-->
    <el-form-item class="global-form-item-margin" v-if="checkPermission('msg:send:log:export')">
  ...
  <el-table :data="tableData"
    ...
    <el-table-column prop="tempName" label="模板名称" align="center" min-width="180"/>
    <el-table-column prop="sendType" label="发送类型" align="center" min-width="100"/>
    <el-table-column prop="sendTarget" label="发送对象" align="center" min-width="180"/>
    <el-table-column prop="sendTime" label="发送时间" align="center" min-width="180"/>
    ...
    <el-table-column prop="errInfo" label="错误信息" align="center" min-width="400"/>
...
import {deleteMsgSendLog, deleteMsgSendLogAll, exportMsgSendLog, getMsgSendLogList} from "@/api/tool/msgSendLog";
import {sendTemplateStatus} from "@/views/tool/msgSendTemplate/dictionary.ts";
...
onMounted(() => {
  if (route.params.name !== 'log') {
    //携带参数
    query.value.tempName = <string>route.params.name;
  }
  getData();
});
...
const getData = () => {
  loading.value = true;
  getMsgSendLogList(paramBuilder(query.value, queryPage.value, time.value, null)).then(res => {
...
const handleExport = () => {
  exportMsgSendLog(null).then(res => {
    downloadExport(res);
  });
}

const handleBack = () => {
  router.push({name: '消息模板'})
}
</script>
