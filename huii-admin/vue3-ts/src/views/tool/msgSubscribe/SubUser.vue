<template>
  ...
  <el-form :inline="true" :size="size">
    ...
    <!--add-->
    <el-form-item class="global-form-item-margin" v-if="checkPermission('tool:msg:sub:query')">
    ...
    <el-form-item class="global-form-item-margin" v-if="checkPermission('tool:msg:sub:edit')">
      <el-button :size="size" :icon="Delete" @click="handleDelete"
                 :color="layoutStore.BtnDelete" plain :disabled="selectable">取消订阅
      </el-button>
    </el-form-item>
    <!--right fixed-->
  ...
  <el-table :data="tableData"
    ...
    <el-table-column label="订阅操作" align="center" width="120" fixed="right"
                     v-if="checkPermissions(['tool:msg:sub:edit'])">
...
import {queryNonSubUser, querySubUser, subUser, unsubUser} from "@/api/tool/msgSubscribe";
...
const getData = () => {
  loading.value = true;
  querySubUser(paramBuilder(query.value, queryPage.value, null, null), sid.value).then(res => {
...
const getData2 = () => {
  loading2.value = true;
  queryNonSubUser(paramBuilder(query2.value, queryPage2.value, null, null), sid.value).then(res => {
    const response = res.data;
...
const handleSubmitForm = (fr: FormInstance | undefined) => {
  ...
  subUser(ids, sid.value).then(res => {
    if (res.code === 0) {
      getData();
      isEdit.value = false;
      dialogVisible.value = false;
...
const handleDelete = (index, row) => {
  ...
  ElMessageBox.confirm(
      '将取消用户名称为  \'' + names.toString().substring(0, 40) + '\'  的用户的订阅，是否确认？',
      '提示', {confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning'}
  ).then(() => {
    unsubUser(ids, sid.value).then((res) => {
      if (res.code === 0) {
        getData();
      }
    });
